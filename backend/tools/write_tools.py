...
from backend.domain.models import EvidenceArtifact, EvidenceStatus, ProposedAction, ActionType, ActionStatus, ApprovalRequest, ApprovalStatus, ActionExecution, AuditEvent, ObligationStatus
...
def attach_evidence(obligation_id: str, name: str, content_type: str, file_path_or_url: str, raw_data_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    evidence = EvidenceArtifact(id=evidence_id, obligation_id=obligation_id, name=name, content_type=content_type, file_path_or_url=file_path_or_url, raw_data_summary=raw_data_summary, status=EvidenceStatus.UNVERIFIED)
    ...
    except Exception as e: return {"error": f"State transition failed: {str(e)}"}
    audit_id = f"clauserunner-audit-{uuid.uuid4().hex[:8]}"
    repo.save_audit_event(AuditEvent(id=audit_id, contract_id=obligation.contract_id, obligation_id=obligation_id, action_type="evidence_attached", description=f"Evidence '{name}' attached. State transitioned from {old_status.value} to {obligation.status.value}.", user_or_system="System"))
...
def record_audit_event(contract_id: str, obligation_id: str, action_type: str, description: str, user_or_system: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    event = AuditEvent(id=event_id, contract_id=contract_id, obligation_id=obligation_id, action_type=action_type, description=description, user_or_system=user_or_system, metadata=metadata or {})
...
def propose_action(obligation_id: str, title: str, description: str, action_type: str, cost_or_impact: str, recipient: str, subject: str, body: str) -> Dict[str, Any]:
    ...
    repo.save_obligation(obligation)
    audit_id = f"clauserunner-audit-{uuid.uuid4().hex[:8]}"
    repo.save_audit_event(AuditEvent(id=audit_id, contract_id=obligation.contract_id, obligation_id=obligation_id, action_type="action_proposed", description=f"Proposed action '{title}' ({action_type}). Requires approval: {requires_approval}.", user_or_system="System"))
    return proposed_action.model_dump(mode="json")
...
def request_approval(proposed_action_id: str, requested_by: str) -> Dict[str, Any]:
    ...
    if not action: return {"error": f"Proposed action '{proposed_action_id}' not found."}
    approvals = repo.list_approval_requests()
    existing_approvals = [a for a in approvals if a.proposed_action_id == proposed_action_id]
    if existing_approvals:
        p_or_a = next((r for r in existing_approvals if r.status in {ApprovalStatus.PENDING, ApprovalStatus.APPROVED}), existing_approvals[0])
        res = p_or_a.model_dump(mode="json")
        res["reused_existing_approval"] = True
        return res
    approval_id = f"clauserunner-approval-{uuid.uuid4().hex[:8]}"
    request = ApprovalRequest(id=approval_id, proposed_action_id=proposed_action_id, requested_by=requested_by, status=ApprovalStatus.PENDING)
    repo.save_approval_request(request)
    obligation = repo.get_obligation(action.obligation_id)
    repo.save_audit_event(AuditEvent(id=f"clauserunner-audit-{uuid.uuid4().hex[:8]}", contract_id=obligation.contract_id if obligation else "unknown", obligation_id=action.obligation_id, action_type="approval_requested", description=f"Requested human approval for action '{action.title}'.", user_or_system=requested_by))
...
def execute_approved_action(proposed_action_id: str, executed_by: str) -> Dict[str, Any]:
    ...
    if action.status == ActionStatus.EXECUTED:
        return {"id": f"clauserunner-exec-{action.id[:8]}", "proposed_action_id": proposed_action_id, "status": "success", "reused_existing_execution": True, "already_executed": True}
    obligation = repo.get_obligation(action.obligation_id)
    if not obligation: return {"error": f"Obligation '{action.obligation_id}' not found."}
    if action.requires_approval:
        ...
        if not matching_approval: return {"error": "EXECUTION BLOCKED: Human approval is strictly required but has not been granted."}
    old_status = obligation.status
    try: ObligationStateMachine.validate_transition(old_status, ObligationStatus.COMPLETED)
    except Exception as e: return {"error": f"State transition validation failed: {str(e)}. Cannot execute."}
    ...
    execution = ActionExecution(id=exec_id, proposed_action_id=proposed_action_id, executed_by=executed_by, status="success", result_artifact={"executed_payload": action.draft_payload, "dispatched_at": datetime.utcnow().isoformat()})
    repo.save_action_execution(execution)
    repo.save_audit_event(AuditEvent(id=f"clauserunner-audit-{uuid.uuid4().hex[:8]}", contract_id=obligation.contract_id, obligation_id=obligation.id, action_type="action_executed", description=f"Successfully executed action '{action.title}' by {executed_by}.", user_or_system=executed_by))
